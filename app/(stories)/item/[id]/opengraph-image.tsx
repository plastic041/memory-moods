...
import { db, usersTable, categoriesTable } from "@/app/db/schema";
  ...
  const story = (
    await db
      .select({
        ...
        comments_count: categoriesTable.comments_count,
        author: categoriesTable.submitted_by,
        domain: categoriesTable.domain,
        submitted_by: usersTable.username,
        username: categoriesTable.username,
      ...
      .leftJoin(
        usersTable,
        sql`${usersTable.id} = ${categoriesTable.submitted_by}`
      )
  )[0];
